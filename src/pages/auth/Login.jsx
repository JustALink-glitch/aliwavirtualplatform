...
// Correct role → dashboard routing
const ROLE_ROUTES = {
...
export default function Login({ portalType = 'admin' }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // First login → must set password regardless of role
    ...
    <div className="min-h-screen bg-[#F1F5F9] flex flex-col font-[Manrope,sans-serif]">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-10">
        <h1 className="text-2xl font-bold text-gray-900 mb-1">
          {portalType === 'trainer' ? 'Tutor Portal' : portalType === 'student' ? 'Student Portal' : 'Sign in'}
        </h1>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 w-full max-w-sm p-6">
          ...
          {/* Google — UI only, OAuth not implemented yet */}
          <button disabled className="w-full flex items-center justify-center gap-2 border border-gray-200 rounded-lg py-2.5 text-sm text-gray-400 cursor-not-allowed opacity-60">
